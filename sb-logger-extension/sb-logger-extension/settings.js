      ...
      forceClvCheckBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          if (response?.success) {
            ...
            let errorMsg = '❌ CLV check failed';
            let color = '#dc3545';
            
            if (response?.error) {
              // Check if error contains "unsupported league" or is purely unsupported leagues
              const isUnsupportedOnly = response.error.includes('unsupported league') && 
                                        !response.error.includes('not found') && 
                                        !response.error.includes('download failed');
              
              if (isUnsupportedOnly) {
                errorMsg = `ℹ️ ${response.error} (cups, international competitions, or non-European leagues not covered by CSV data)`;
                color = '#666';
              } else {
                errorMsg = `❌ CLV check failed: ${response.error}`;
            ...
            if (resultDiv) {
              resultDiv.innerHTML = `<span style="color: ${color}; white-space: pre-line;">${errorMsg}</span>`;
            }
            alert(errorMsg);
          }
        } catch (err) {
