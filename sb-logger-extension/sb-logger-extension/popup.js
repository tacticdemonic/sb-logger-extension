...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function checkClvEligibility(bet) {
    ...
    // Step 3: Check if tournament is supported (use existing functions if available)
    if (bet.tournament && typeof isUnsupportedTournament === 'function') {
      if (isUnsupportedTournament(bet.tournament)) {
        return {
          ...
          tooltip: `Tournament "${bet.tournament}" not covered by football-data.co.uk (international/cup competitions excluded)`
        };
      }
    }
    ...
    // Step 5: Check if market type is supported
    if (bet.market) {
