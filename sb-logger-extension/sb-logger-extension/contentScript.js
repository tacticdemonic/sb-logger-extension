...
function () {
  ...
  const CSS = `
    ...
    .surebet-helper-clv-tile.na {
      background: #f8d7da !important;
      color: #721c24 !important;
    }
    .surebet-helper-clv-tile.ok {
      background: #d4edda !important;
      color: #155724 !important;
    }
  `;
  ...
  function ensureClvTile(row, parentEl) {
    if (!row || !parentEl) {
      return null;
    }
    let tile = row.querySelector('.surebet-helper-clv-tile');
    if (!tile) {
      tile = document.createElement('span');
      tile.className = 'surebet-helper-clv-tile';
      parentEl.appendChild(tile);
    }
    return tile;
  }

  function isMarketClvAvailable(marketText) {
    const text = String(marketText || '').trim();
    if (!text) {
      return false;
    }
    try {
      // Reuse the existing CLV whitelist preset as the definition of “CLV available”.
      const compiled = COMPILED_MARKET_PATTERNS?.clv_available;
      if (compiled && compiled.pattern && typeof compiled.pattern.test === 'function') {
        return compiled.pattern.test(text);
      }
      const preset = MARKET_FILTER_PRESETS?.clv_available;
  ...
  function updateRowClvTile(row, betData) {
    if (!row || !betData) {
      return;
    }
    const parentEl = row.querySelector('td .d-flex');
    ...
    const tile = ensureClvTile(row, parentEl);
    if (!tile) {
      return;
    }
    const clvAvailable = isMarketClvAvailable(betData.market);
    tile.classList.toggle('na', !clvAvailable);
    tile.classList.toggle('ok', clvAvailable);
    if (!clvAvailable) {
      tile.textContent = 'CLV N/A';
      tile.title = 'CLV not available for this market (unsupported market type)';
    } else {
      tile.textContent = 'CLV ✓';
      tile.title = 'CLV available for this market';
    }
  }
  ...
  function updateStakeIndicators() {
    ...
    rows.forEach((row) => {
      if (row.__surebetHelperBetData) {
        ...
        updateRowClvTile(row, row.__surebetHelperBetData);
      }
    });
  }
