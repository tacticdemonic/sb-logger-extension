...
class ApiService {
  ...
  async fetchOtherSportsResults(sport, date) {
    ...
    const cacheKey = `other_${sportKey}_${date.toISOString().split('T')[0]}`;
    console.log('🏀 Fetching results for:', sport, '(', sportKey, ')');
    const cached = this.getCached(cacheKey);
    if (cached) return cached;

    try {
      console.log('🌐 Making Odds API request...');
      const response = await fetch(
      ...
      const data = await response.json();
      console.log('✅ Odds API response:', data?.length || 0, 'results');
  ...
  matchFootballEvent(bet, fixtures) {
    ...
    const betTeams = betEvent.split(/\s*[–—-]\s*/);
    ...
    const [team1, team2] = betTeams.map(t => t.trim());
